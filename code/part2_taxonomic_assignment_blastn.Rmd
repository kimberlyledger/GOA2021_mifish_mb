...
- this is the fasta seqs from the 20241106 run on dadasnake, and blastn using a ncbi nt database accessed on 2024-07-18 with 96% seq identity and 98% query coverage   
```{r load-output-from-taxonkit}
taxonomy <-read.delim("/home/kimberly.ledger/GOA2021_mifish_mb/data/blastn_taxlineage.txt", header = FALSE, na.strings=c(""," ","NA"))
...
tax_df <- taxonomy %>%
  ...
  filter(perc_id > 98) %>% # blast output includes as low as 96 perc_id, but for now i only care about the top 3%
...
spp_range <- spp_range %>%
  ...
  mutate(in_range = ifelse(Species == "Coregonus steinmanni", "no", in_range)) 
...
## read in the custom gadid db results and compare/join to full taxonomy 
```{r}
gadidae <- read.table("/home/kimberly.ledger/GOA2021_mifish_mb/data/gadidae_results.txt")  %>%
  filter(V4 > 100) %>% # make sure all retained matches are >100 bp
  ...
  filter(perc_id > 98) %>%
  filter(species != "Arctogadus_glacialis") %>%  ## polar cod is not in GOA so going to remove it
  filter(species != "Boreogadus_saida")  ## also removing arctic cod, very low chances of getting it in GOA 
```

## rank the hits by % identity
```{r}
seq_id_diff_gadidae <- gadidae %>%
  select(-length) %>%
...
```{r}
keepers_gadidae <- seq_id_diff_gadidae %>%
  filter(diff < 0.5)
```
...
## so now determine whether ASVs should be assigned to genus, family, order, etc. 
```{r}
singletons_gadidae <- keepers_gadidae %>%
  select(qseqid) %>%
  tally() %>%
  filter(n == 1)

# these are the seqs that now have only a single match
singleton_df_gadidae <- singletons_gadidae %>%
  left_join(keepers_gadidae) %>%
  select(-n) %>%
  mutate(taxonomic_level = "species") %>%
  mutate(taxon = species)
...
## Genus-level matches
# remove the singletons from the bigger df 
single_genus_gadidae <- keepers_gadidae %>%
  anti_join(singleton_df_gadidae)%>% # 
  left_join(., keepers_gadidae) %>%
...
gadus <- keepers_gadidae %>%
  anti_join(singleton_df_gadidae)%>% # 
  left_join(., keepers_gadidae) %>% 
  filter(qseqid %in% single_genus_gadidae$qseqid) %>%
  mutate(taxon = "Gadus") %>%
  mutate(taxonomic_level = "genus")
  
## what's remaining? 
leftover_gadidae <- keepers_gadidae %>%
  anti_join(singleton_df_gadidae)%>% # 
  left_join(., keepers_gadidae) %>%
  filter(!qseqid %in% single_genus_gadidae$qseqid) %>%
...
* note that now all "Gadidae" are between saffron cod and tomcod. 

remove underscore from taxon name 
```{r}
gadidae_df <- singleton_df_gadidae %>%
  bind_rows(gadus) %>%
  bind_rows(leftover_gadidae)
...
# this may include ASVs that are better matches to non-gadids so now let's filter by the ASVs assigned to gadidae using ncbi nt db 
gadidae_asvs <- tax_df_2 %>%
  filter(family == "Gadidae") %>%
  select(qseqid) %>%
  unique()

gadidae_df_filtered <- gadidae_df %>%
  filter(qseqid %in% gadidae_asvs$qseqid)
```

## read in custom salmon db results and compare/join to full taxonomy 
```{r}
salmon <- read.table("/home/kimberly.ledger/GOA2021_mifish_mb/data/oncorhynchus_results.txt")  %>%
  filter(V4 > 100) %>% # make sure all retained matches are >100 bp
  select(-V5, -V6, -V7, -V8, -V9, -V10, -V11, -V12, -V13, -V14) %>%  #remove unnecessary columns
  group_by(V1, V2) %>% # group by the sequence key and the full taxonomy to reduce duplicate entries
  unique() %>% # doing that reduced the number of entries
...
# this may include ASVs that are better matches to non-gadids so now let's filter by the ASVs assigned to gadidae using ncbi nt db 
salmon_asvs <- tax_df_2 %>%
  filter(genus == "Oncorhynchus") %>%
  select(qseqid) %>%
  unique()

salmon_df_filtered <- salmon_df %>%
  filter(qseqid %in% salmon_asvs$qseqid)
```

## replace gadidae and salmon ids in tax_df_2 with results from custom dbs
```{r}
tax_df_3 <- tax_df_2 %>%
  filter(family != "Gadidae") %>%
  filter(genus != "Oncorhynchus") %>%
  bind_rows(gadidae_df_filtered) %>%
  bind_rows(salmon_df_filtered) %>%
  arrange(qseqid)
```

## out of curiousity, compare the difference pre/post custom dbs
```{r}
tax_df_2 %>%
  anti_join(tax_df_3)
...
```{r}
write.csv(tax_df_3, "/home/kimberly.ledger/GOA2021_mifish_mb/outputs/taxonomy_20241107_uncollapsed.csv")
write.csv(tax_df_4, "/home/kimberly.ledger/GOA2021_mifish_mb/outputs/taxonomy_20241107_collapsed.csv")
```
